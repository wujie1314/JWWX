...
function initXiala(){
	 mescroll = initMeScroll("mescroll", {
		...
		up: {
			...
			callback: getListData, //上拉回调,此处可简写; 相当于 callback: function (page) { (page); }
		}
	});
}
...
function addInvitationDiv(data){
	...
	InvitationDiv += "<div class='foot' style='margin-left:57px;margin-top:10px;'>"+"<span>当前评论："+data[4] +"| </span>" + "<span onclick='particulars("+ data[0] +")'> 详情</span>" + "</div>";
	InvitationDiv += "</div>";
	$("#leirong").append(InvitationDiv);
}
