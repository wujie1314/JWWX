...
.toPhoneNum {
	...
	<div class="rightPart">
		<div class="inputForm">
			...
			<button type="submit" style="width:150px;" class="btn btn-default"
				id="confirmLogin " onclick="enter()">登录</button>
		</div>
	</div>
</body>
...
<script>
   ...
   window.onload=function(){//只有第一次加载时，才使用这个方法
         init();
   }
	function init(){
	...
	function check(value) {
		$.ajax({
			...
			success : function(o) {
				...
				switch (result) {
				case "1":
					window.location.href = "/login?userName=" + value.ACCOUNT;
					break;
				case "-1"://dataError
	...
	function enter() {
		...
		$.ajax({
			...
			success : function(o) {
				...
				switch (result) {
				case "-9":
					alert("用户名不能为空"); //userError
					window.location.href = "/skip/userError";
					break;
				case "-6":
					alert("密码不能为空"); //deptError
